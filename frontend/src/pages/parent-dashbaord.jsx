...
function ParentDashboard() {
  ...
  const actionItems = (parentProgress?.action_items || []).map((item) => ({
    id: item.id,
    title: item.title,
  ...
  const dashboardTaskPreview = sortedActionItems.slice(0, 5);
  const hasMoreTasks = actionItems.length > 5;
          ...
          >
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "2rem" }}>
                <Link
                  to={selectedChild?.id ? `/smartgoals?studentId=${selectedChild.id}` : "/smartgoals"}
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                {dashboardTaskPreview.length > 0 ? dashboardTaskPreview.map((item) => (
                  <div
                    key={item.id}
                    style={{
                      backgroundColor: "#fff",
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: "0.5rem" }}>
                      <input
                        ...
                        readOnly
                        style={{ width: "20px", height: "20px", marginRight: "0.75rem", marginTop: "0.125rem", flexShrink: 0 }}
                      />
                    ...
                    <div style={{ color: "#999", fontSize: "0.85rem", marginTop: "0.5rem" }}>
                      📅 Due: {formatDueDateTime(item.due_date, item.due_time)}
                    </div>
                  </div>
                )) : (
                  <div style={{ backgroundColor: "#fff", padding: "1rem", borderRadius: "6px", color: "#666" }}>
                ...
                {hasMoreTasks && (
                  <button
                    type="button"
                    onClick={() =>
                      navigate(selectedChild?.id ? `/tasks?studentId=${selectedChild.id}` : "/tasks")
                    }
                    style={{
                      marginTop: "0.5rem",
                      padding: "0.8rem 1rem",
